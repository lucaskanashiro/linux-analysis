...
t = 1:length(cwe476)
par(mfrow = c(2, 2)) 
hist(cwe476,main="",xlab="CWE476",ylim=c(0,130),xlim=c(0.005,0.012))
qqnorm(cwe476, ylab="CWE476 Quantiles", xlab="Normal Quantiles");qqline(cwe476)
plot(t,cwe476,ylab="CWE476",xlab="Versions",type="l", xlim=c(0,395), ylim=c(0.0055,0.012))
plot(cwe476,Lag(cwe476),xlab="cwe476[i-1]",ylab="cwe476[i]",ylim=c(0.005,0.012),xlim=c(0.005,0.012))
mtext("CWE 476 Data: 4-Plot", line = 0.5, outer = TRUE)

# Generate boxplot - CWE476
par(mfrow=c(1,1))
boxplot(cwe476, main="CWE 476", ylim=c(0.0055,0.012))
...
plot(training$modules, training$cwe476, xlab="Modules", 
     ylab="tax_CWE476", main="LOESS - CWE476", ylim=c(0.0055,0.012),
     xlim=c(14000,38000))
...
plot(training$modules, training$cwe476, xlab="Modules", 
     ylab="taxCWE476", main="Quadratic Model - CWE476", ylim=c(0.0055,0.012),
     xlim=c(14000,38000))
...
plot(training$modules, training$cwe476, xlab="Modules", 
     ylab="taxCWE476", main="Cubic Model - CWE476", ylim=c(0.0055,0.012),
     xlim=c(14000,38000))
...
# ANOVA analysis
anova(quadratic_model, cubic_model, test="F")
...
cubic_cv <- CVlm(df=testing, 
                 form.lm=formula(cwe476 ~ modules + I(modules^2) + I(modules^3)),
                 m=10, main="Validação Cruzada k-fold - Modelo grau 3")
quadratic_cv <- CVlm(df=testing, 
                     form.lm=formula(cwe476 ~ modules + I(modules^2)), 
                     m=10, main="Validação Cruzada k-fold - Modelo grau 2")

require(boot)

model2 <- glm(cwe476 ~ poly(modules, 2), data=data)
model3 <- glm(cwe476 ~ poly(modules, 3), data=data)
model4 <- glm(cwe476 ~ poly(modules, 4), data=data)
model5 <- glm(cwe476 ~ poly(modules, 5), data=data)
model6 <- glm(cwe476 ~ poly(modules, 6), data=data)
model7 <- glm(cwe476 ~ poly(modules, 7), data=data)
model8 <- glm(cwe476 ~ poly(modules, 8), data=data)
model9 <- glm(cwe476 ~ poly(modules, 9), data=data)
model10 <- glm(cwe476 ~ poly(modules, 10), data=data)
model11 <- glm(cwe476 ~ poly(modules, 11), data=data)
model12 <- glm(cwe476 ~ poly(modules, 12), data=data)
model13 <- glm(cwe476 ~ poly(modules, 13), data=data)
model14 <- glm(cwe476 ~ poly(modules, 14), data=data)
model15 <- glm(cwe476 ~ poly(modules, 15), data=data)
model16 <- glm(cwe476 ~ poly(modules, 16), data=data)
model17 <- glm(cwe476 ~ poly(modules, 17), data=data)
model18 <- glm(cwe476 ~ poly(modules, 18), data=data)
model19 <- glm(cwe476 ~ poly(modules, 19), data=data)
model20 <- glm(cwe476 ~ poly(modules, 20), data=data)
...
plot(squared_err, xlab="Grau do modelo", ylab="Erro de predição", main="Erro de predição - CWE476", xlim=c(1,19))
