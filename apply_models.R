...
i=1
real_cwe476_taxes <- vector(mode="numeric", length=0)
real_cwe457_taxes <- vector(mode="numeric", length=0)
cwe476_model_taxes <- vector(mode="numeric", length=0)
cwe457_model_taxes <- vector(mode="numeric", length=0)

for (cwe in data$CWE476){
	real_cwe476_taxes[i] <- c(n_cwe476[i]/n_modules[i])
	real_cwe457_taxes[i] <- c(n_cwe457[i]/n_modules[i])
...
df = data.frame(kernel_version, n_cwe476, n_modules, cwe476_model_taxes, real_cwe476_taxes)
pdf("cwe476.png", height=4, width=7)
grid.table(df)
dev.off()


df2 = data.frame(kernel_version, n_cwe457, n_modules, cwe457_model_taxes, real_cwe457_taxes)
